...
- Интеграция с LLM через OpenRouter, модель выбирается через переменные
  окружения.
- Использование простых Markdown-правил из `rules/` как runtime-контекста для
  LLM при интерпретации запросов, выборе сценария КП и уточнениях.
- Определение намерения менеджера через LLM tool/action selection для обычного
  текста: работа с черновиками, изменение расчёта, добавление позиций, создание
  КП или уточнение.
...
- Локальное хранилище SQLite для пользователей, сообщений, черновиков КП и
  выбранных позиций.
- Просмотр и открытие незавершённых черновиков КП пользователя, когда у
  разговора нет активного черновика.
- Поддержка правил лицензирования, апгрейдов, бандлов и поведения бота через
  человекочитаемые Markdown-файлы.
...
- Строгий DSL, YAML-схема или отдельный rule engine для предметных правил.
- Редактирование правил через административный интерфейс.
...
- Бот учитывает правила из `rules/`, например выбор КОРП-позиций при большом
  количестве пользователей, варианты апгрейда и бандлы.
- Бот отличает просьбу показать или открыть черновики от запроса на поиск
  продуктов и в этом случае читает SQLite, а не вызывает MCP.
